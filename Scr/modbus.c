#include "modbus.h"
#include "eemem.h"

__flash const unsigned char srCRCHi[256] = {  };
__flash const unsigned char srCRCLo[256] = {  };

struct D_pc_str D_pc;
//extern __flash const unsigned char srCRCHi[256];

unsigned int GetCRC16(unsigned char *buf, unsigned char bufsize) 
{
	uint8_t CRC_Low = 0xFF, CRC_High = 0xFF, k, carry;

	for (k = 0; k < bufsize; k++) 
        {
		carry = CRC_Low ^ buf[k];
		CRC_Low = CRC_High ^ srCRCHi[carry];
		CRC_High = srCRCLo[carry];
	}
	return (((int) CRC_High << 8) | CRC_Low);
}

void Link_To_PC(void) 
{
	uint8_t NumWords, reg, i = 0;
	uint8_t err = 0;//,  dv, lbm;  //bbn,

	if(D_pc.Flags_Link_To_PC&(1<<RECIVED_POCET_FROM_PC)) 
        {
		D_pc.Flags_Link_To_PC&=~(1<<RECIVED_POCET_FROM_PC);
                
		if(   D_pc.Counter_Byte_To_PC>=7 && D_pc.mr[0]==ADRESS_DEVICE
		   && (*(unsigned int*)&D_pc.mr[D_pc.Counter_Byte_To_PC-2]==GetCRC16((void*)&D_pc.mr,D_pc.Counter_Byte_To_PC-2)) ) 
                {

//#define mr D_pc.mr
//#define mw D_pc.mw
//#define mwh D_pc.Num_Byte_RW_To_PC

			switch (D_pc.mr[FUNCTION_MODBUS]) 
                        {
			case 3:
				reg = D_pc.mr[2] << 8 | D_pc.mr[3];
				NumWords = D_pc.mr[4] << 8 | D_pc.mr[5];

				if (reg < NUMBER_OF_RW_REGS)
                                {
                                  if(       (NumWords+reg > NUMBER_OF_RW_REGS) ||  !NumWords
                                        ||  (NumWords > NWBUF)) { err=3; break;}
				  for(i=0; i < NumWords; i++) 
                                  {
				    D_pc.mw[i*2+3] = D_pc.ModBUS_rw_reg[reg + i] >> 8;
				    D_pc.mw[i*2+1+3] = D_pc.ModBUS_rw_reg[reg + i];
				  }
				}
                                else { err=2; break; }
                                
				D_pc.mw[0] = D_pc.mr[0];
				D_pc.mw[1] = D_pc.mr[1]; //func
				D_pc.mw[2] = NumWords << 1; //eie-ai aaeo
				D_pc.Num_Byte_RW_To_PC = (NumWords << 1) + 3;
				break;
			case 4:
				reg = D_pc.mr[2] << 8 | D_pc.mr[3];
				NumWords = D_pc.mr[4] << 8 | D_pc.mr[5];

				if (reg < NUMBER_OF_R_REGS) 
                                {
					if (NumWords + reg > NUMBER_OF_R_REGS || !NumWords
							|| NumWords > NWBUF) {
						err = 3;
						break;
					}
					for (i = 0; i < NumWords; i++) 
                                        {
						D_pc.mw[i * 2 + 3] = D_pc.ModBUS_r_reg[reg + i] >> 8;
						D_pc.mw[i * 2 + 1 + 3] = D_pc.ModBUS_r_reg[reg + i];
					}
				}                                        
				else {	err=2; break; }
				D_pc.mw[0] = D_pc.mr[0];
				D_pc.mw[1] = D_pc.mr[1]; //func
				D_pc.mw[2] = NumWords << 1; //eie-ai aaeo
				D_pc.Num_Byte_RW_To_PC = (NumWords << 1) + 3;
				break;       
                                
			case 16:
				reg = D_pc.mr[2] << 8 | D_pc.mr[3];
				NumWords = D_pc.mr[4] << 8 | D_pc.mr[5];
				if(reg < NUMBER_OF_RW_REGS) 
        {
				  if(NumWords+reg > NUMBER_OF_RW_REGS || !NumWords || NumWords > NWBUF) { err=3; break; }
				  for(i=0; i < NumWords; i++) 
          {
				      unsigned char ai=i*2 + 7,
						    aa=reg+i;			
				    if(D_pc.ModBUS_rw_reg[aa] != (unsigned int)D_pc.mr[ai]<<8 | D_pc.mr[ai+1]) 
            {
              if (aa < NUM_NOT_TO_EE)
              {
                D_pc.ModBUS_rw_reg_write_flags[aa >> 3] |= 1<< (aa & 7);
                eeprom_write_byte((aa<<1) + START_ADR_EE_DATA, D_pc.mr[ai]);					
                eeprom_write_byte((aa<<1) + START_ADR_EE_DATA + 1, D_pc.mr[ai+1]);	
              }
				      D_pc.ModBUS_rw_reg[aa] = D_pc.mr[ai]<<8 | D_pc.mr[ai+1];
				    }
				  }
				}                                 
        else { err=2; break; }
				D_pc.Num_Byte_RW_To_PC=6;
				for(i=0; i < D_pc.Num_Byte_RW_To_PC; i++) D_pc.mw[i]=D_pc.mr[i];
				break;

			default:
				err = 1;
			} //switch
			if (err) {
				D_pc.mw[0] = D_pc.mr[0];
				D_pc.mw[1] = D_pc.mr[1] | 0x80;
				D_pc.mw[2] = err;
				D_pc.Num_Byte_RW_To_PC = 3;
			}

			*(unsigned int *) &D_pc.mw[D_pc.Num_Byte_RW_To_PC] = GetCRC16((void *) &D_pc.mw, D_pc.Num_Byte_RW_To_PC);
			D_pc.Num_Byte_RW_To_PC += 2;

			uint8_t tt;
			for(tt = 0; tt<NWBUF; tt++){
				D_pc.mr[tt]=0;
			}
			REG_UCSRB |= (1<<B_TXEN|1<<B_TXCIE);
			REG_UDR = D_pc.mw[0];
		} else {
			REG_UCSRB |= (1<<B_RXEN|1<<B_RXCIE);
		}
		D_pc.Counter_Byte_To_PC = 0;
	}
}
//#undef mw
//#undef mr
